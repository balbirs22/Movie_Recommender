...
def fetch_poster(movie_id):
    response = requests.get('https://api.themoviedb.org/3/movie/{}?api_key=7dd8d47a64ea6b48964d56f40ff6873b&&language=en-US'.format(movie_id))
    data = response.json()
...
st.title("Balbir's Movie Recommender System")
selected_movie_name = st.selectbox("Select a movie:", movies_list)

if st.button('Recommend'):
    names, posters = recommend(selected_movie_name, movies_df)
